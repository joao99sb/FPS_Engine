...
double fFOV = 3.14159265358979 / 4.0;
float fDepth = 16.0f;
...
int main(int argc, char const *argv[]) {
	...
	while (1)
	{
		...
		tp1 = tp2;
		float fElapsedTime = elapsedTime.count() * 3 ; 
		


		//controls
		//handle ccw rotation
		if (GetAsyncKeyState((short)'A') & 0x8000) {
			fPlayerA -= (1.0f) * fElapsedTime;
		}
		if (GetAsyncKeyState((short)'D') & 0x8000) {
			fPlayerA += (1.0f) * fElapsedTime;
		}
		...
		//Exit the game
		if (GetAsyncKeyState((short)'B') & 0x8000) {
			break;
		}
		...
		WriteConsole(hConsole, screen, nScreenHeight * nScreenWidth, &dwBytesWritten, NULL);
